...
export default class ItemsLoader<T> {
    ...
    constructor(page: number, itemsPerPage: number, apiPath: string, constantFilters: Record<string, any> = {}) {
        this.page = page
    ...
    async reload() {
        ...
        try {
            this.startTimeout()
            const newItems = await $fetch<ListReturn<T>>(this.apiPath, {
                method: "GET",
                query: {
                    page: this.page,
    ...
    /**
     * Загружает первую страницу используя useFetct nuxt'a.
     * В связи с этим первую страницу нужно помещать в items самостоятельно. 
     * @returns то, что возращает useFetch 
     */
    async loadFirstPage() {
        const fetchObj = useFetch<ListReturn<T>>(this.apiPath,
            {
                method: "GET",
                query: {
                    page: this.page,
